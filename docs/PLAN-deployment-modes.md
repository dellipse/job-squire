...
The LinuxServer base brings a conventional `PUID`, `PGID`, and `UMASK` model, which aligns with how the current image already parameterizes the non-root user through build arguments and `data/.env`. The container continues to run the application as a non-root user, and the shared `/data` volume continues to be owned by that user so it stays readable and writable across updates. `/data` is a named Docker volume, not a bind-mounted host directory (WAL-mode SQLite over a bind mount bridged through OrbStack/Docker Desktop's VM filesystem layer intermittently throws "disk I/O error" under concurrent access; a named volume is native daemon-managed storage and doesn't hit that class of bug), so this ownership is re-applied by the image's own `init-data-dir` s6 service on every boot rather than assumed to already be correct -- that matters in particular right after a restore, when `job-squire restore`/`scripts/restore.sh` populate a freshly created, empty volume via `docker cp` as root. Only `data/.env` is still a plain host file, layered on top of the volume as a single-file bind mount, since `env_file:` has to read it before the volume even exists.
...
Backup produces a single self-contained archive of one instance, written to the user's home folder, as a `.tgz` by default with a `.zip` option, named for the instance and a UTC timestamp (for example `job-squire-castelo-20260711T1830Z.tgz`). The archive contains everything the data directory would hold if it were still all host-visible, including any files the application itself does not manage, so nothing is lost in a backup. Capture is WAL-safe: the same Online Backup API mechanism the app's own in-app download uses so the database is captured in a consistent state, reusing the existing WAL-safe approach rather than inventing a new one. `/data` became a named Docker volume rather than a bind mount (Section 2) after this section was first drafted, so in practice the CLI gathers the two halves differently: config files (`docker-compose.yml`, the compose-level `.env`, `data/.env`) are still walked directly on the host, but the database/uploads/etc. are pulled live out of the running container via `docker exec` (see `app/backup_cli.py`) rather than read off a host path -- which is also why backup now requires the instance's container to be running. By default it backs up one instance; an option can back up every registered instance in one run.
...
Restore takes an archive and recreates the instance: it prompts for the archive passphrase and decrypts it, failing clearly if the passphrase is wrong, then verifies the checksums and backup-format compatibility, restores the config files and `data/.env` (including `SECRET_KEY`) straight to the host, re-registers the instance from the manifest, keeps or reallocates ports and hostname as appropriate for the target machine, ensures a runtime is available, creates the container (without starting it) so its named volume exists, `docker cp`s the restored database/uploads/etc. into that volume, then brings the container up on a compatible image version, letting the app's additive migrations carry the schema forward if the target is newer. Creating the container before copying the data in avoids any race with the app's own first-boot database creation; the image's `init-data-dir` s6 service re-owns everything under `/data` on the next boot regardless of who wrote it. If an instance of the same name already exists, the CLI prompts to rename or overwrite rather than clobbering it silently.
...
- **Base image and Dockerfile.** Replace the `python:3.14-slim` base with the LinuxServer `baseimage-alpine` base, pinned to a dated tag, installing Python via `apk add python3 py3-pip`, and add the branding file plus `ENV LSIO_FIRST_PARTY=false`. Originally pinned to the Alpine 3.23 line (Python 3.12); bumped to the 3.24 line (Python 3.14) on 2026-07-17 to pick up a curl/libcurl CVE fix (CVE-2026-5773, CVE-2026-6276) with no backport on 3.23. Both Python jumps were verified the same way before being trusted: the full lockfile, `cryptography` included, installs on musl with `pip install --only-binary=:all:` (binary wheels for every package, no source builds) and imports cleanly at runtime, so the musl/wheel-coverage risk stays retired across the bump; `pydantic` and `pydantic-core` remain pinned to keep resolution deterministic. The Debian-slim (with `tini` and s6 or supervisord) or LinuxServer Ubuntu fallback is retained only as a contingency.
...
| musl and Python version verification | **Resolved (2026-07-11):** verified for the Alpine 3.23 base (Python 3.12) — full lockfile resolves to musllinux binary wheels for all 61 packages with no source builds, `cryptography` included. **Re-verified (2026-07-17):** same check repeated for the Alpine 3.24 base (Python 3.14) before bumping to it for a curl CVE fix — `pip install --only-binary=:all:` passed for all 61 packages and runtime imports of flask/sqlalchemy/cryptography/lxml/pydantic_core/mcp were confirmed clean inside the built image. Pin `pydantic`/`pydantic-core`. Debian-slim/Ubuntu fallback kept only as a contingency. See Section 2. |
