...
Check the Anthropic API key on Settings > AI tab (it is a separate pay-per-use key, not a Claude subscription). Verify the model string -- the default is `claude-sonnet-4-6`. The specific error class is shown in the flashed UI message.
