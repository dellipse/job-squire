...
| `TRIAGE_INTERVAL_MINUTES` | `5` | Minutes between standalone auto-triage checks for unscored Saved jobs, independent of the search schedule (catches jobs added via MCP `add_jobs`). `0` disables. |
| `HEARTBEAT_INTERVAL_MINUTES` | `5` | How often the worker touches `DATA_DIR/.worker_heartbeat`, backing the aggregated healthcheck and the in-app staleness warning. |
...
| `AI_HTTP_TIMEOUT` | `55` | Seconds before a cloud AI provider call times out. |
| `AI_LOCAL_HTTP_TIMEOUT` | `300` | Seconds before a local provider (Ollama/LiteLLM) call times out — longer, since local inference is slower. |
...
**Automated AI feature toggles** (API mode only, each stored in `AIConfig`). These are legacy
per-feature toggles, kept as a fallback for installs that predate the per-task provider assignment
in `AITaskConfig` (AI tab → Automatic Features — see below); a task with an `AITaskConfig` row uses
that row's `enabled` flag instead:
...
| `provider` | Type slug: `gemini`, `groq`, `openrouter`, `ollama`, `litellm`, `mistral`, `openai`, `cerebras`, `github_models`, `nous_portal`, `anthropic`, `custom`. Determines the built-in base URL. |
| `label` | Optional display name (e.g. "Gemini Free Tier"). |
| `api_key_enc` | API key, Fernet-encrypted at rest. Not required for Ollama or LiteLLM. |
| `base_url` | Override URL. Required for Ollama, LiteLLM, and custom endpoints; leave blank for cloud providers. |
...
### Automatic feature provider assignment — `AITaskConfig` (AI tab → Automatic Features)

Each automated task (triage, followup, weekly_review, rejection_alert) has its own row: a primary
provider, an optional backup provider (tried if the primary fails), and a "continue down the ranked
chain" checkbox if both fail. This takes precedence over the legacy per-feature toggles below,
which are now fallbacks kept for migration from pre-multi-provider installs.
