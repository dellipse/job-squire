...
def save_resume_draft(resume_markdown: str, profile_facts: str = "",
                      created_by: str = "") -> dict:
    """Persist a completed resume from the onboarding resume interview.
    ...
    docs/PLAN-onboarding.md Phase 2. Upserts the single kind="Resume"
    CandidateAsset (AI-generated, distinct from an uploaded "Base Resume")
    so re-running the interview replaces the previous draft instead of
    piling up duplicates. Optionally merges extracted profile facts
    (background, skills, etc.) into candidate_profile.md.
    ...
    stored_name = asset.stored_name if asset else f"{uuid.uuid4().hex}.md"
    ...
    if asset:
        asset.original_name = "resume.md"
        asset.content_type = "text/markdown"
        asset.size = size
        asset.notes = "Generated by the Getting Started resume interview."
    else:
