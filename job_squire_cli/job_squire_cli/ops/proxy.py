...
def render_swag_compose(*, network: str, timezone: str, url: str, validation: str) -> str:
    """A minimal, standalone SWAG compose file.

    DNS/certificate validation (DuckDNS, Cloudflare DNS-01, ...) is Prompt
    C10's job -- `url`/`validation` here are just SWAG's own required env
    vars, passed through with whatever the operator already has (or empty/
    "http" placeholders C10 overwrites once a real domain is in hand).
    Network mode is still not considered configured without a working
    proxy in front (PLAN Section 5), and a bare SWAG container with no
    valid domain yet is exactly that -- present, but not yet finished.
    """
    return f"""\
